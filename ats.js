...
async function exportarPdfAts(id) {
  ...
  const filasHtml = (ficha.filas || []).map((f, i) => `
    <tr>
      <td style="text-align:center;width:28px;border:1px solid #bbb;padding:6px 4px;vertical-align:top;font-weight:700">${i+1}</td>
      <td style="border:1px solid #bbb;padding:7px 8px;vertical-align:top;line-height:1.5">${textoAPdf(f.paso)}</td>
      <td style="border:1px solid #bbb;padding:7px 8px;vertical-align:top;line-height:1.5">${textoAPdf(f.peligro)}</td>
      <td style="border:1px solid #bbb;padding:7px 8px;vertical-align:top;line-height:1.5">${textoAPdf(f.control)}</td>
    </tr>`).join('');
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;padding:16px;color:#000;font-size:11px">
      <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #2d4a6e;padding-bottom:10px;margin-bottom:14px">
        <div style="font-size:18px;font-weight:900;color:#2d4a6e">SGAS</div>
        <div style="text-align:center">
          <div style="font-size:13px;font-weight:700;color:#1e2640">ANÁLISIS DE TRABAJO SEGURO (ATS)</div>
          <div style="font-size:10px;color:#555;margin-top:2px">${escapeHtml(ficha.categoria)}</div>
        </div>
        <div style="text-align:right;font-size:10px;color:#555">${fechaHoy}</div>
      </div>
      <div style="margin-bottom:12px;padding:8px;background:#f0f4f8;border-radius:4px">
        <strong>Tarea:</strong> ${escapeHtml(ficha.nombre)}
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead>
          <tr>
            <th style="background:#2d4a6e;color:#fff;padding:8px 6px;border:1px solid #bbb;width:28px;text-align:center">#</th>
            <th style="background:#2d4a6e;color:#fff;padding:8px;border:1px solid #bbb;width:28%">Pasos de la tarea<br><span style="font-weight:400;font-size:9px;opacity:.8">Describe los pasos a seguir</span></th>
            <th style="background:#2d4a6e;color:#fff;padding:8px;border:1px solid #bbb;width:36%">Peligros identificados<br><span style="font-weight:400;font-size:9px;opacity:.8">Detalla los peligros asociados</span></th>
            <th style="background:#2d4a6e;color:#fff;padding:8px;border:1px solid #bbb;width:36%">Medidas de control<br><span style="font-weight:400;font-size:9px;opacity:.8">Acciones para prevenir o mitigar</span></th>
          </tr>
        </thead>
